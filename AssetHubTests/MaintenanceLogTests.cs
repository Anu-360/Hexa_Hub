...
namespace AssetHubTests
{
    public class MaintenanceLogTests
    {
        ...
        public async Task UpdateLog()
        {
            ...
            _repoMock.Setup(ml=>ml.UpdateMaintenanceLog(Log))
                .ReturnsAsync(Log);

            var result = await _repo.UpdateMaintenanceLog(Log);

            Assert.IsNotNull(result, "Result should not be null");
            Assert.AreEqual(Log.MaintenanceId, result.MaintenanceId, " Id Should Match");
            Assert.AreEqual(Log.Maintenance_Description, result.Maintenance_Description, " Desc Should Match");
        }
