 ...
 useEffect(() => {
  ...
  const openPrompt = (asset) => {
    if (asset.assetStatusName === 'Allocated') {
      ...
      return;
    }else if (asset.assetStatusName === 'UnderMaintenance'){
      setErrorMessage(`Asset "${asset.assetName}" is under maintenance. Please select available assets.`);
      return; 
    }
...
const toggleAssetStatusDropdown = () => {
  ...
  const handleAssetStatusSelect = (status) => {
    ...
    const filtered = assetsData.filter(asset => asset.assetStatusName === status);
    setFilteredAssets(filtered);
  };
